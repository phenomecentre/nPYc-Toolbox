...
class MSDataset(Dataset):
	...
	* XCMS
		...
		Note that retention time is specified in seconds by default.

	* XCMSonline
		XCMS online result download supplies an unannotated and an annotated xlsx file stored by default in "XCMS results" folder. However, these excel files are usually too large to be imported by python modules and excel file structure does not allow chunkwise import.
		Therefore, the respective results xlsx file MUST be converted to csv format manually. Note that delimiter is a comma symbol ","
		By default, the table is expected to have 10 columns of feature parameters, with the intensity values for the first sample coming on the 11th column. However, the number of columns to skip is dataset dependent and can be set with the (e ``noFeatureParams=`` keyword argument.
		Note that retention time is specified in minutes by default and that all Sample File Names have been already converted to lower case letters by XCMSonline processing. Adjust Sample File Names in Sample Metadata file accordingly if needed.

	* MZmine
		MZmine import operates on csv files export via the 'Export to CSV' file' menu option. Field separator should be comma "," and all export elements should be chosen for export.

	* MS-DIAL
		MS-DIAL import operates on txt (msp) files that exported via the 'Export -> Alignment result' menu option. Export options to choose are preferably 'Raw data matrix (Area)' or 'Raw data matrix (Height)'.
		Note that import will copy metadata information such as File Type, Injection Order and Batch ID. These can however be added and overwritten by importing separate metadata file.
	...
	* nPYc
	...
	def _loadMZmineDataset(self, path):
		...
		# Get the sample names as the only metadata we have
		sampleMetadata = dict()
		sampleMetadata['Sample File Name'] = [name[:-10] for name in list(values.columns.values)]
