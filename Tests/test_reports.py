...
from nPYc.enumerations import VariableType, AssayRole, SampleType, QuantificationType, CalibrationMethod, \
    AnalyticalPlatform
from nPYc.reports._generateReportTargeted import _generateReportTargeted, _postMergeLOQDataset, \
    _prePostMergeLOQSummaryTable, _getAccuracyPrecisionTable, _getRSDTable
...
class test_reports_ms_feature_id(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.msData.sampleMetadata['Acquired Time'] = [d for d in datetime_range(datetime.now(), self.msData.noSamples,
...
class test_reports_generateSamplereport(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.data.addSampleInfo(descriptionFormat='NPC LIMS',
                                filePath=os.path.join('..', '..', 'npc-standard-project', 'Derived_Worklists',
                                                      'UnitTest1_MS_serum_PCSOP.069.csv'))
        self.data.addSampleInfo(descriptionFormat='NPC Subject Info',
                                filePath=os.path.join('..', '..', 'npc-standard-project', 'Project_Description',
                                                      'UnitTest1_metadata_PCDOC.014.xlsx'))
    ...
    def test_report_samplesummary_postexclusion(self):
        ...
        self.data.excludeSamples(
            self.data.sampleMetadata.iloc[self.data.sampleMetadata['Skipped'].values == True]['Sample File Name'],
            on='Sample File Name', message='Skipped (_x)')
        self.data.excludeSamples(
            self.data.sampleMetadata[pandas.isnull(self.data.sampleMetadata['Sample Base Name'])]['Sample File Name'],
            on='Sample File Name', message='Unknown type')
...
class test_reports_nmr_generatereport(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dataset = generateTestDataset(self.noSamp, self.noFeat, dtype='NMRDataset',
                                           variableType=VariableType.Continuum, sop='GenericNMRurine')
    ...
    def test_report_nmr_features(self):
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            expectedPath = os.path.join(tmpdirname, 'graphics', 'report_featureSummary',
                                        'TestData_calibrationCheck.png')
            self.assertTrue(os.path.exists(expectedPath))

            expectedPath = os.path.join(tmpdirname, 'graphics', 'report_featureSummary',
                                        'TestData_finalFeatureBLWPplots1.png')
            self.assertTrue(os.path.exists(expectedPath))

            expectedPath = os.path.join(tmpdirname, 'graphics', 'report_featureSummary',
                                        'TestData_finalFeatureBLWPplots3.png')
            self.assertTrue(os.path.exists(expectedPath))

            expectedPath = os.path.join(tmpdirname, 'graphics', 'report_featureSummary',
                                        'TestData_peakWidthBoxplot.png')
            self.assertTrue(os.path.exists(expectedPath))
    ...
    def test_report_nmr_final(self):
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            expectedPath = os.path.join(tmpdirname, 'graphics', 'report_finalSummary',
                                        'TestData_spectraSolventPeakRegion.png')
            self.assertTrue(os.path.exists(expectedPath))
...
class test_reports_ms_generatereport(unittest.TestCase):
    ...
    def test_reports_ms_featuresummary(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            testFiles = ['test_RSDdistributionFigure.png', 'test_TICinLR.png', 'test_acquisitionStructure.png',
                         'test_correlationByPerc.png', 'test_ionMap.png', 'test_meanIntensityFeature.png',
                         'test_meanIntensitySample.png',
    ...
    def test_reports_ms_raises(self):
        ...
        self.assertRaises(TypeError, nPYc.reports.generateReport, data, 'batch correction summary',
                          msDataCorrected='Not an object')
...
class test_reports_targeted_generatereport(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.targetedDataset.expectedConcentration = pandas.DataFrame(numpy.array([[1., 2.], [4., 5.], [7., 8.]]),
        ...
        self.tDataAccPrec.sampleMetadata = pandas.DataFrame(
            {'AssayRole': [AssayRole.PrecisionReference, AssayRole.PrecisionReference,
        ...
        self.tDataAccPrec.sampleMetadata['Sample File Name'] = ['UnitTest_targeted_file_001',
                                                                'UnitTest_targeted_file_002',
                                                                'UnitTest_targeted_file_003',
        ...
        self.tDataAccPrec.sampleMetadata['Sample Name'] = ['Sample1', 'Sample2', 'Sample3', 'Sample4', 'Sample5',
                                                           'Sample6', 'Sample7', 'Sample8']
        self.tDataAccPrec.sampleMetadata['Acqu Date'] = ['10-Sep-16', '10-Sep-16', '10-Sep-16', '10-Sep-16',
                                                         '10-Sep-16', '10-Sep-16', '10-Sep-16', '10-Sep-16']
        self.tDataAccPrec.sampleMetadata['Acqu Time'] = ['03:23:02', '04:52:35', '05:46:40', '06:10:43', '07:30:06',
                                                         '08:50:29', '09:05:42', '09:20:58']
        self.tDataAccPrec.sampleMetadata['Instrument'] = ['XEVO-TQS#UnitTest', 'XEVO-TQS#UnitTest', 'XEVO-TQS#UnitTest',
                                                          'XEVO-TQS#UnitTest',
                                                          'XEVO-TQS#UnitTest', 'XEVO-TQS#UnitTest', 'XEVO-TQS#UnitTest',
                                                          'XEVO-TQS#UnitTest']
        ...
        self.tDataAccPrec.sampleMetadata['Dilution'] = [numpy.nan, numpy.nan, numpy.nan, numpy.nan, numpy.nan,
                                                        numpy.nan, numpy.nan, numpy.nan]
        self.tDataAccPrec.sampleMetadata['Correction Batch'] = [numpy.nan, numpy.nan, numpy.nan, numpy.nan, numpy.nan,
                                                                numpy.nan, numpy.nan, numpy.nan]
        ...
        self.tDataAccPrec._intensityData = numpy.array(
            [[10., 10., 10.], [5., 50., 5.], [15., 90., 15.], [95., 30., 95.], [100., 30., 100], [105., 50., 105.],
             [100., 101., 100], [95., 105., 95]])
        self.tDataAccPrec.expectedConcentration = pandas.DataFrame(
            numpy.array([[10., 50., 10.], [10., 50., 10.], [10., 50., 10.],
                         [100., 50., 100.], [100., 50., 100.], [100., 50., 100.],
                         [numpy.nan, numpy.nan, numpy.nan], [numpy.nan, numpy.nan, numpy.nan]]),
            columns=self.tDataAccPrec.featureMetadata['Feature Name'].values.tolist())
        ...
        self.tDataAccPrec.calibration['calibFeatureMetadata'] = pandas.DataFrame(
            {'Feature Name': ['Feature1', 'Feature2', 'Feature3']})
        self.tDataAccPrec.calibration['calibExpectedConcentration'] = pandas.DataFrame(
        ...
        self.resAcc = pandas.DataFrame(
            {10.0: [100.0, numpy.nan, 100.0, numpy.nan, numpy.nan, numpy.nan, 100.0, numpy.nan, 100.0],
             ...
             'Feature': ['Feature1', 'Feature1', 'Feature1', 'Feature2', 'Feature2', 'Feature2', 'Feature3', 'Feature3',
                         'Feature3'],
        ...
        self.resPre = pandas.DataFrame(
            {10.0: [40.8248, numpy.nan, 40.8248, numpy.nan, numpy.nan, numpy.nan, 40.8248, numpy.nan, 40.8248],
        ...
        self.resAccPre = pandas.DataFrame(
            {'1': [100.0, numpy.nan, 100.0, numpy.nan, numpy.nan, numpy.nan, 100.0, numpy.nan, 100.0],
        ...
        tuplesCol = [(10.0, 'Acc.'), (10.0, 'Prec.'), (50.0, 'Acc.'), (50.0, 'Prec.'), (100.0, 'Acc.'),
                     (100.0, 'Prec.')]
        colIdx = pandas.MultiIndex.from_tuples(tuplesCol, names=['Concentration', 'Measure'])
        self.resAccPre.index = colIdx
        self.resAccPre = self.resAccPre.transpose()
        tuplesRow = [('Feature1', 'Study Pool'), ('Feature1', 'External Reference'), ('Feature1', 'All Samples'),
                     ('Feature2', 'Study Pool'),
                     ('Feature2', 'External Reference'), ('Feature2', 'All Samples'), ('Feature3', 'Study Pool'),
                     ('Feature3', 'External Reference'),
    ...
    def test_generateReportTargeted_raises(self):

        self.assertRaises(ValueError, nPYc.reports._generateReportTargeted, self.targetedDataset, 'nonsense')
        self.assertRaises(TypeError, nPYc.reports._generateReportTargeted, self.targetedDataset, "feature summary", destinationPath=3)
        self.assertRaises(TypeError, nPYc.reports._generateReportTargeted, self.targetedDataset, "feature summary", percentRange="string")
    ...
    def test_report_postMergeLOQDataset(self):
        ...
        pandas.testing.assert_frame_equal(result.calibration['calibSampleMetadata'],
                                          expected.calibration['calibSampleMetadata'])
        pandas.testing.assert_frame_equal(result.calibration['calibFeatureMetadata'],
                                          expected.calibration['calibFeatureMetadata'])
        pandas.testing.assert_frame_equal(result.calibration['calibExpectedConcentration'],
    ...
    def test_report_getaccuracyprecisiontable(self):
        ...
        pandas.testing.assert_frame_equal(resultAcc, expectedAcc, check_dtype=False, check_index_type=False,
                                          check_column_type=False, check_like=False)
        # Precision Table
        pandas.testing.assert_frame_equal(resultPre, expectedPre, check_dtype=False, check_index_type=False,
                                          check_column_type=False, check_like=False)
        # Both Tables
        pandas.testing.assert_frame_equal(resultAccPre, expectedAccPre, check_dtype=False, check_index_type=False,
                                          check_column_type=False, check_like=False)
    ...
    def test_reports_targeted_featuresummary(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            with warnings.catch_warnings():
                warnings.simplefilter('ignore', UserWarning)
                nPYc.reports.generateReport(inputDataset, 'feature summary', withExclusions=True, destinationPath=tmpdirname)

            expectedPath = os.path.join(tmpdirname, 'unittest_report_featureSummary.html')
            self.assertTrue(os.path.exists(expectedPath))

            testFiles = ['unittest_FeatureAccuracy-A.png', 'unittest_FeatureAccuracy-B.png',
                         ...
                         'unittest_FeaturePrecision-A.png', 'unittest_FeaturePrecision-B.png']
    ...
    def test_reports_targeted_finalreport(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirPCA:
            ...
            pcaModel = nPYc.multivariate.exploratoryAnalysisPCA(inputDataset, withExclusions=True, scaling=1.0,
                                                                maxComponents=2)

        with tempfile.TemporaryDirectory() as tmpdirname:

            nPYc.reports.multivariateReport(inputDataset, pcaModel=pcaModel, reportType='analytical',
                                            withExclusions=True, destinationPath=tmpdirPCA)
            ...
            multivariateTestFiles = ['unittest_PCAloadingsPlot_PCAloadings.png',
                                     'unittest_PCAscoresPlot_SampleClassPC1vsPC2.png']
    ...
    def test_reports_targeted_raises(self):
        ...
        self.assertRaises(TypeError, nPYc.reports.generateReport, inputDataset, 'feature summary',
                          withExclusions='Not a bool')
        ...
        self.assertRaises(TypeError, nPYc.reports.generateReport, inputDataset, 'feature summary',
                          numberPlotPerRowLOQ='Not an int')
        # numberPlotPerRowFeature is not an int
        self.assertRaises(TypeError, nPYc.reports.generateReport, inputDataset, 'feature summary',
                          numberPlotPerRowFeature='Not an int')
...
class test_reports_modules(unittest.TestCase):
    ...
    def test_reports_final_peakpanther(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            expectedPath = os.path.join(tmpdirname,
                                        'graphics/report_finalSummary/UnitTest1_PCSOP.069_QI_finalFeatureIntensityHist.png')
            self.assertTrue(os.path.exists(expectedPath))
    ...
    def test_reports_generatefeaturereport(self):
        ...
        for i in range(noGroups):
            ...
            for j in range(noPlots):
                ...
                reportingGroups[groupName][randomword(10)] = dataset.featureMetadata['Feature Name'].iloc[
                    featureIndexes].values
    ...
    def test_reports_generatefeaturereport_filterUnits(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            # Build reports
            report = generateFeatureDistributionReport(dataset, reportingGroups, filterUnits='Unit B',
                                                       destinationPath=tmpdirname)
...
class test_reports_multivariate(unittest.TestCase):
    ...
    def test_multivariateqcreport_all(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            with self.subTest(msg='PCA object generation'):
                ...
                numpy.testing.assert_equal(pcaModel._npyc_dataset_shape['NumberSamples'],
                                           self.dataset.intensityData.shape[0])
                numpy.testing.assert_equal(pcaModel._npyc_dataset_shape['NumberFeatures'],
                                           self.dataset.intensityData.shape[1])

            # Report generation
            with self.subTest(msg='Report generation'):
                nPYc.reports.multivariateReport(self.dataset, pcaModel=pcaModel, reportType='all',
                                                destinationPath=tmpdirname)

                expectedPath = os.path.join(tmpdirname, 'graphics', 'report_multivariateAll')
                self.assertTrue(os.path.exists(expectedPath))
                testFiles = ['Dataset_PCAloadingsPlot_PCAloadingsPC1.png', 'Dataset_PCAloadingsPlot_PCAloadingsPC2.png',
                             'Dataset_PCAscoresPlot_SampleClassPC1vsPC2.png',
                             'Dataset_PCAscreePlot.png', 'Dataset_metadataPlot_metadataDistribution_categorical0.png',
                             'Dataset_metadataPlot_metadataDistribution_continuous0.png',
                             'Dataset_metadataPlot_metadataDistribution_date0.png', 'Dataset_modOutliersPlot.png',
                             'Dataset_sigCorHeatmap.png', 'Dataset_sigKruHeatmap.png',
    ...
    def test_multivariateqcreport_analytical(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            self.dataset.Attributes['analyticalMeasurements'] = {'SampleType': 'categorical',
                                                                 'AssayRole': 'categorical', 'Acquired Time': 'date',
                                                                 'Run Order': 'continuous', 'Dilution': 'continuous',
                                                                 'Detector': 'continuous',
    ...
    def test_multivariateqcreport_raises(self):
        ...
        self.assertRaises(TypeError, nPYc.reports.multivariateReport, self.dataset, pcaModel,
                          withExclusions='not a bool')

        self.assertRaises(TypeError, nPYc.reports.multivariateReport, self.dataset, pcaModel,
                          biologicalMeasurements='not a dict')

        self.assertRaises(ValueError, nPYc.reports.multivariateReport, self.dataset, pcaModel, reportType=1)

        self.assertRaises(ValueError, nPYc.reports.multivariateReport, self.dataset, pcaModel, dModX_criticalVal=0.05,
                          dModX_criticalVal_type=None)

        self.assertRaises(ValueError, nPYc.reports.multivariateReport, self.dataset, pcaModel, dModX_criticalVal=-1,
                          dModX_criticalVal_type='Fcrit')

        self.assertRaises(ValueError, nPYc.reports.multivariateReport, self.dataset, pcaModel, dModX_criticalVal=101,
                          dModX_criticalVal_type='Fcrit')

        self.assertRaises(ValueError, nPYc.reports.multivariateReport, self.dataset, pcaModel,
                          dModX_criticalVal_type=-1)

        self.assertRaises(ValueError, nPYc.reports.multivariateReport, self.dataset, pcaModel,
                          dModX_criticalVal_type='wrong threshold')
